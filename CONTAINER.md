# teleinfo2mqtt Container Images

Container images based on [teleinfo2mqtt](https://github.com/fmartinou/teleinfo2mqtt), a service that reads Linky smart meter data (Télé-Information Client) and publishes it to an MQTT broker.

Sources are available on [GitHub](https://github.com/anthochamp/container-teleinfo2mqtt).

See [README.md](README.md) for full documentation and configuration reference.

## Image tags

- `x.y.z-teleinfo2mqttA.B.C`: Container image version `x.y.z` with teleinfo2mqtt `A.B.C`.
- `edge-teleinfo2mqttA.B.C`: Latest commit build with teleinfo2mqtt `A.B.C`.

**Tag aliases:**

- `x.y-teleinfo2mqttA.B.C`: Latest patch of `x.y` with teleinfo2mqtt `A.B.C`.
- `x-teleinfo2mqttA.B.C`: Latest minor+patch of `x` with teleinfo2mqtt `A.B.C`.
- `x.y.z`: Version `x.y.z` with latest teleinfo2mqtt (only latest container version updated).
- `x.y`: Latest patch of `x.y` with latest teleinfo2mqtt (only latest container major.minor updated).
- `x`: Latest minor+patch of `x` with latest teleinfo2mqtt (only latest container major updated).
- `teleinfo2mqttA.B`: Latest container with latest patch of teleinfo2mqtt `A.B`.
- `teleinfo2mqttA`: Latest container with latest minor+patch of teleinfo2mqtt `A`.
- `latest`: Latest `x.y.z-teleinfo2mqttA.B.C` tag.
- `edge-teleinfo2mqttA.B`: Latest commit build with latest patch of teleinfo2mqtt `A.B`.
- `edge-teleinfo2mqttA`: Latest commit build with latest minor+patch of teleinfo2mqtt `A`.
- `edge`: Latest `edge-teleinfo2mqttA.B.C` tag.
